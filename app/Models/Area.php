...
{
    try {
        return self::with(['city.state.country']) // Eager load city, state, and country
            ->when(
                !empty($search['name']),
                fn($query) => $query->where(function ($q) use ($search) {
                    $q->where('name', 'like', "%" . trim($search['name']) . "%") // Search in areas
                      ->orWhereHas('city', fn($q2) => 
                          $q2->where('name', 'like', "%" . trim($search['name']) . "%") // Search in cities
                      )
                      ->orWhereHas('city.state', fn($q3) => 
                          $q3->where('name', 'like', "%" . trim($search['name']) . "%") // Search in states
                      )
                      ->orWhereHas('city.state.country', fn($q4) => 
                          $q4->where('name', 'like', "%" . trim($search['name']) . "%") // Search in countries
                      );
                })
            )
            ->when(

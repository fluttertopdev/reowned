...
class Itempackage extends Model
{
    ...
    {
        return $this->hasMany(UserPayment::class, 'item_package_id');
    }


    public function translation()
    {
        return $this->hasOne(ItempackageTranslation::class, 'itempackage_id')
    ...
    public static function getLists($search, $status = null)
    {
        try {
            return self::query()
                ...
                // If $status param is NOT NULL → apply status = 1
                ->when(
                    !is_null($status),
                    fn($query) =>

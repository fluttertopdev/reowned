        ...
        <div class="card mb-6">
            ...
            <form id="categoryForm" class="card-body"
                ...
                @if($isSubcategory)
                    ...
                    <hr class="my-4">
                    <h5 class="mb-3">{{__('lang.custom_fields')}}</h5>
    ...
    document.addEventListener('DOMContentLoaded', function () {
        ...
        form.addEventListener('submit', function(e) {
            ...
            if (!hasExistingImage && !newImageSelected) {
                ...
                Swal.fire({
                    icon: 'warning',
                    title: "{{__('lang.image_required')}}",
                    text: "{{__('lang.image_required_msg')}}"
                });
            }
    ...
    function appendField(data = null) {
        ...
        <div class="card p-3 mb-3 custom-field-item" data-index="${index}">
            <div class="row g-3">
                ...
                <div class="col-md-3">
                    <label>{{ __('lang.field_name') }}</label>
                    <input type="text" class="form-control field-name"
                        value="${data ? data.field_name : ''}">
                </div>

                <div class="col-md-3">
                    <label>{{ __('lang.field_type') }}</label>
                    <select class="form-control field-type">
                        <option value="textbox">{{ __('lang.text_input') }}</option>
                        <option value="number">{{ __('lang.number_input') }}</option>
                        <option value="dropdown">{{ __('lang.dropdown') }}</option>
                        <option value="checkbox">{{ __('lang.checkbox') }}</option>
                    </select>
                </div>
                ...
                <div class="col-md-12 options-section d-none">
                    <label>{{ __('lang.field_values') }}</label>
                    <input type="text" class="form-control option-input"
                        placeholder="{{ __('lang.type_value_press_enter') }}">

    ...
    <div class="container-xxl flex-grow-1 container-p-y">
        <div class="card mb-6">
            ...
            <form class="card-body"
                action="{{isset($data) ? route('staffCategory.update') : route('staffCategory.store') }}"
                ...
                <div class="row g-6">
                    <div class="col-md-6">
                        <label class="form-label">{{__('lang.category_name')}}</label>
                        <span class="text-danger">*</span>
                        <input type="text" id="name" name="name" class="form-control"
                            value="{{ old('name', $data->name ?? '') }}"
                            onkeyup="generateSlug()" placeholder="{{__('lang.category_name')}}" />
                        @error('name')
                ...
                <div class="pt-6">
                    ...
                    <a href="{{ route('staffCategory.index') }}" class="btn btn-outline-secondary">{{__('lang.back')}}</a>
                </div>
            </form>
        </div>
    </div>

            ...
            <div class="card-header">
                <form method="get">
                    <div class="row ">
                        ...
                        <div class="col-md-6">
                             @if (in_array('add-category', \Helpers::staffgetAssignedPermissions()))
                            <div class="table-btn-css">
                                <a href="{{ isset(request()->category) ? route('staffCategory.form') . '?category=' . request()->category : route('staffCategory.form') }}">
                        ...
                        <div class="col-sm-3 display-inline-block mt-3">
                            ...
                            <a type="reset" class="btn btn-outline-secondary"
                                href="{{ route('staffCategory.index', ['category' => request()->get('category')]) }}">
                                {{__('lang.reset')}}
                            </a>
                            @else
                            <a type="reset" class="btn btn-outline-secondary" href="{{ route('staffCategory.index') }}">{{__('lang.reset')}}</a>
                            @endif
                        </div>
                    </div>
                </form>
                    ...
                    <table class="table">
                        <thead class="table-light">
                            <tr class="text-nowrap">
                                ...
                                <th>{{__('lang.created_at')}}</th>
                                 @if (in_array('category-status', \Helpers::staffgetAssignedPermissions()))
                        ...
                        <tbody>
                            ...
                            <tr>
                                ...
                                <td>
                                    <a href="{{ route('staffCategory.index', ['category' => $row->id]) }}">
                                ...
                                <td>{{ \Helpers::commonDateFormate($row->created_at) }}</td>
                                 @if (in_array('category-status', \Helpers::staffgetAssignedPermissions()))
                                <td>
                                    <a href="{{ route('staffCategory.updateStatus', $row->id) }}">
                                ...
                                <td>
                                    <div class="dropdown">
                                        ...
                                        <div class="dropdown-menu">
                                            @if(!isset(request()->category))
                                             @if (in_array('category-status', \Helpers::staffgetAssignedPermissions()))
                                            <a href="{{ route('staffCategory.index', ['category' => $row->id]) }}" class="dropdown-item" type="button">
                                                <i class="ti ti-eye me-1"></i>{{__('lang.view')}}
                                            </a>
                                            @endif
                                            @endif
                                             @if (in_array('update-category', \Helpers::staffgetAssignedPermissions()))
                                            <a href="{{ request()->has('category') 
                          ? route('staffCategory.form', $row->id) . '?category=' . request()->category 
                                  : route('staffCategory.form', $row->id) }}"
                                                class="dropdown-item">
                                             ...
                                             @endif
                                              ...
                                              @if (in_array('delete-category', \Helpers::staffgetAssignedPermissions()))
                                            ...
                                            <!-- Show 'Add Subcategory' for Categories only -->
                                              @if (in_array('add-subcategory', \Helpers::staffgetAssignedPermissions()))
                                            <a href="{{ route('staffCategory.index', ['category' => $row->id]) }}" class="dropdown-item">

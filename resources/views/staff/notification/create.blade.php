...
<div class="content-wrapper">
    <div class="container-xxl flex-grow-1 container-p-y">
        ...
        <div class="card mb-6">
            ...
            <form class="card-body" action="{{ isset($data) ? route('staffNotification.update') : route('staffNotification.store') }}" method="POST" enctype="multipart/form-data" id="description-form">
                @csrf
                @if(isset($data))
                <input type="hidden" name="id" value="{{ $data->id }}">
                @endif
                  <input type="hidden" name="staff_id" value="{{ Auth::guard('staff')->check() ? Auth::guard('staff')->id() : '' }}">
                <div class="row g-6">
                    <!-- Title Field -->
                    <div class="col-md-6">
                    ...
                    <div class="col-md-6">
                        ...
                        <span class="text-danger">*</span>
                        <textarea type="text" id="multicol-discount" class="form-control" name="msg" placeholder="{{__('lang.message')}}" @if(isset($data)) readonly @endif>{{ old('msg', $data->msg ?? '') }}</textarea>
                        @error('msg')
                        <span class="text-danger">{{ $message }}</span>
                        @enderror
                    </div>
                ...
                <div class="pt-6">
                    ...
                    @endif
                    <a href="{{route('staffNotification.index')}}" class="btn btn-label-secondary">
                        {{__('lang.back')}}
                    </a>
                </div>
            </form>
